 ...
 */
export function validateNetworkConfig(config: any, networkName: string): NetworkConfig {
...
export function validateLaunchpadConfig(config: any): LaunchpadConfig {
  ...
  for (const [networkName, networkConfig] of Object.entries(config.networks)) {
    validatedNetworks[networkName] = validateNetworkConfig(networkConfig, networkName);
  }
 ...
 */
export async function testNetworkConnectivity(networkConfig: NetworkConfig): Promise<boolean> {
  try {
 ...
 */
export async function testAllNetworkConnectivity(config: LaunchpadConfig): Promise<Record<string, boolean>> {
  const results: Record<string, boolean> = {};
  
  const connectivityTests = Object.entries(config.networks).map(async ([name, networkConfig]) => {
