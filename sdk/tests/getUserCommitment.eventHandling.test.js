...
const AuctionAPI_1 = require("../src/core/AuctionAPI");
const events_1 = require("../src/utils/events");
// Mock Solana Web3.js
jest.mock('@solana/web3.js', () => ({
    Connection: jest.fn(),
    PublicKey: jest.fn().mockImplementation((key) => ({
        toString: () => key,
        equals: (other) => other.toString() === key,
...
describe('getUserCommitment Event Handling', () => {
    ...
    let eventParser;
    // Test data
    const programId = new web3_js_1.PublicKey('CwKEDwUYppVotihuZgCdBbF7UHvCUAV2EzMrZ2Ttjnyp');
    const auctionId = new web3_js_1.PublicKey('7aH5Kw2CvJ4DpG3yRm8TnL1qF9bN6eX2sP5wU3vZ4kT9');
    const userKey = new web3_js_1.PublicKey('9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM');
    const committedPda = new web3_js_1.PublicKey('3iKqC9oBAgKH2c4vqDhFHdqNbLhEP4rKoFQJBm9XyKL8');
    const binId = 0;
    // Mock event data
    const mockCommittedAccountSnapshot = {
        auction: auctionId,
        user: userKey,
        bins: [
            {
                binId: 0,
                paymentTokenCommitted: new bn_js_1.default('3000000'),
                saleTokenClaimed: new bn_js_1.default('3000000'),
            },
            {
                binId: 1,
                paymentTokenCommitted: new bn_js_1.default('2000000'),
                saleTokenClaimed: new bn_js_1.default('2000000'),
            },
        ],
        bump: 255,
        totalPaymentCommitted: new bn_js_1.default('5000000'),
        totalSaleTokensClaimed: new bn_js_1.default('5000000'),
    };
    const mockClosureEvent = {
        userKey,
        auctionKey: auctionId,
        committedAccountKey: committedPda,
        rentReturned: new bn_js_1.default('2039280'),
    ...
    beforeEach(() => {
        ...
        auctionAPI = new AuctionAPI_1.AuctionAPI(mockSDK);
        eventParser = new events_1.EventParser(mockConnection, programId);
    ...
    describe('when Committed account is closed', () => {
        ...
        it('should find and parse CommittedAccountClosedEvent for specific bin', async () => {
            ...
            expect(mockConnection.getSignaturesForAddress).toHaveBeenCalledWith(committedPda, { limit: 2 });
            ...
            expect(result === null || result === void 0 ? void 0 : result.binId).toBe(0);
            expect(result === null || result === void 0 ? void 0 : result.paymentTokenCommitted.toString()).toBe('3000000');
            expect(result === null || result === void 0 ? void 0 : result.saleTokenClaimed.toString()).toBe('3000000');
            // Verify console logs were called for event details
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('⚠️  Committed account not found - checking for account closure event'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('🔍 Searching for account closure event for PDA'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('✅ Found account closure event!'));
        });
        it('should return null for non-existent bin in closure event', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining(`No commitment found for bin ${nonExistentBinId} in closure event`));
        });
        ...
        it('should handle case when no signatures are found', async () => {
            ...
            const result = await auctionAPI.getUserCommitment(auctionId, userKey, binId);
            expect(result).toBeNull();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('No transactions found for this PDA'));
        });
        it('should handle case when no CommittedAccountClosedEvent is found', async () => {
            ...
            const result = await auctionAPI.getUserCommitment(auctionId, userKey, binId);
            expect(result).toBeNull();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('No CommittedAccountClosedEvent found in any transaction'));
        });
    });
    describe('when Committed account exists', () => {
        ...
        it('should parse account data normally without checking events', async () => {
            ...
            expect(result).toBeDefined();
            expect(result === null || result === void 0 ? void 0 : result.binId).toBe(0);
            expect(result === null || result === void 0 ? void 0 : result.paymentTokenCommitted.toString()).toBe('1000000');
            expect(result === null || result === void 0 ? void 0 : result.saleTokenClaimed.toString()).toBe('0');
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Committed account found - parsing account data'));
        });
    });
    describe('error handling', () => {
        ...
        it('should handle connection errors gracefully', async () => {
            ...
            expect(console.error).toHaveBeenCalledWith('Error in getUserCommitment:', expect.any(Error));
        });
        it('should handle transaction parsing errors', async () => {
            ...
            const result = await auctionAPI.getUserCommitment(auctionId, userKey, binId);
            expect(result).toBeNull();
            expect(console.error).toHaveBeenCalledWith('Error finding CommittedAccountClosedEvent:', expect.any(Error));
        });
    });
    describe('EventParser direct usage', () => {
        it('should print detailed event information', () => {
            ...
            events_1.EventParser.printCommittedAccountClosedEvent(mockClosureEvent);
            expect(consoleSpy).toHaveBeenCalledWith('🎯 ===== CommittedAccountClosedEvent Found =====');
            expect(consoleSpy).toHaveBeenCalledWith(`📋 Transaction Signature: ${mockClosureEvent.signature}`);
            expect(consoleSpy).toHaveBeenCalledWith(`👤 User: ${mockClosureEvent.userKey.toString()}`);
            expect(consoleSpy).toHaveBeenCalledWith(`💰 Rent Returned: ${mockClosureEvent.rentReturned.toString()} lamports`);
